...
- Учитывай уровень навыков пользователя: не предлагай действия, которые явно выше его опыта без предупреждений и безопасной альтернативы
- Учитывай инвентарь пользователя. Если для ремонта нужны инструменты, которых нет в инвентаре, явно перечисли, что нужно купить или одолжить
- В каждом ответе первой отдельной строкой пиши название техники строго в формате: "Название техники: <название>". Если техника не определена, пиши: "Название техники: не определено"
`;
...
export interface RepairAssistantUserContext {
...
function buildInstructions(context?: RepairAssistantUserContext): string {
  if (!context) return SYSTEM_PROMPT;

  const skill = context.skillLevelName
    ? `${context.skillLevelName}${context.skillLevelDescription ? ` — ${context.skillLevelDescription}` : ""}`
    : "не указан";
  const tools =
    context.tools.length > 0
      ? context.tools
          .map((tool) => {
            ...
            return `- ${tool.name}: ${tool.quantity} шт.${description}`;
          })
          .join("\n")
      : "Инвентарь пуст или не заполнен.";

  return `${SYSTEM_PROMPT}

Профиль пользователя:
- Уровень навыков: ${skill}
- Текущий инвентарь:
${tools}
`;
}
...
export async function askAIagent(
  history: RepairAssistantHistoryTurn[],
  context?: RepairAssistantUserContext
): Promise<string> {
  if (history.length === 0) {
